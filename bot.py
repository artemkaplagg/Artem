...
2. Если что-то не получилось - дай КОНКРЕТНЫЙ совет, не ругай
3. Дай МОТИВАЦИЮ на завтра (максимум 3-4 предложения)
4. Если прогресс идёт - скажи "ТЫ НА ПРАВИЛЬНОМ ПУТИ"
5. Напиши НА РУССКОМ, эмоционально, как тренер, как друг
...
async def process_report(message: Message):
    ...
    # ФОРМИРУЕМ КОРОТКИЙ CAPTION (макс 1024 символа для Telegram)
    emoji_woke = "✅" if woke_up else "❌"
    emoji_turnik = "✅" if turnik_sets > 0 else "❌"
    emoji_hw = "✅" if homework else "❌"
    emoji_sleep = "✅" if sleep else "❌"

    short_caption = f"📊 <b>ОТЧЁТ {datetime.now().strftime('%d.%m.%Y')}</b>\n\n{emoji_woke} 6:30 | {emoji_turnik} Турник:{turnik_sets} | {emoji_hw} ДЗ | {emoji_sleep} 9PM"
    ...
    # ОТПРАВЛЯЕМ ПОЛНЫЙ ОТЧЁТ ОТДЕЛЬНЫМ СООБЩЕНИЕМ
    full_report = f"""
<b>ПОЛНЫЙ ОТЧЁТ</b>

✅ Встал в 6:30: {'ДА ✅' if woke_up else 'НЕТ ❌'}
💪 Турник: {turnik_sets} подходов
📚 Домашка: {'ГОТОВА ✅' if homework else 'НЕ ГОТОВА ❌'}
😴 Сон в 9 PM: {'ДА ✅' if sleep else 'НЕТ ❌'}
⚡ Доп упражнения: {'ДА ✅' if extra else 'НЕТ ❌'}

<i>Заметки:</i> {notes if notes != 'нет' else '—'}

━━━━━━━━━━━━━━━━━━━━━
🤖 <b>MISTRAL FEEDBACK:</b>
━━━━━━━━━━━━━━━━━━━━━

{ai_feedback}

<i>✨ Напиши /stats для полной статистики!</i>
"""
